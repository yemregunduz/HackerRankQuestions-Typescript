...
class HackerRankSolution {
    ...
    calculateGradingStudents(originalGrades) {
        const finalGrades = [];
    ...
    countApplesAndOranges(regionStart, regionEnd, appleTreeLocation, orangeTreeLocation, distanceOfApplesToAppleTree, distanceOfOrangesToOrangeTree) {
        let result = {
            countOfApples: 0,
            countOfOranges: 0,
        };
        distanceOfApplesToAppleTree.forEach((distance) => {
            if (appleTreeLocation + distance >= regionStart &&
                appleTreeLocation + distance <= regionEnd) {
                result.countOfApples++;
            }
        });
        distanceOfOrangesToOrangeTree.forEach((distance) => {
            if (orangeTreeLocation + distance >= regionStart &&
    ...
    kangaroo(firstKangarooLocation, firstKangarooSpeed, secondKangarooLocation, secondKangarooSpeed) {
        let result = "NO";
        if (secondKangarooSpeed < firstKangarooSpeed) {
            if ((secondKangarooLocation - firstKangarooLocation) %
        ...
        else if (secondKangarooSpeed == firstKangarooSpeed &&
    ...
    subArrayDivision(segments, day, month) {
        ...
        for (let i = 0; i < segments.length; i++) {
            ...
            let sumOfSubArray = subArray.reduce((acumulator, currentValue) => acumulator + currentValue, 0);
